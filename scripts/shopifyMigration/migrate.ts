...
async function fetchRechargeEntityCollection(entityCollectionName: string): Promise<any[]> {
    ...
    let request = fetch(`https://api.rechargeapps.com/${entityCollectionName}`, {
        method: "GET",
...
async function fetchShopifyEntityCollection(entityCollectionName: string): Promise<any[]> {
    ...
    let request = fetch(
        `https://87700134e27c0aca518cb36d356c837b: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@bon-chef-recetas-ingredientes.myshopify.com/admin/api/2021-07/${entityCollectionName}.json`
    );
    let response = await request;
    let value = await response.json();
    let result: any[] = value[entityCollectionName];
