...
async function migrateSubscriptions() {
    ...
    for (const rechargeSubscription of rechargeSubscriptions) {
        ...
        if (!["ACTIVE", "CANCELLED"].includes(rechargeSubscription.status)) return;
        ...
        if (!letsCookVariantId) {
            console.warn(`Couldn't find LC variant for Shopify variant ${shopifyVariantId}. Skipping subscription ${rechargeSubscription.id}.`);
            continue;
        }
        ...
        subscriptionMigrationPromises.push(createSubscription.execute({
